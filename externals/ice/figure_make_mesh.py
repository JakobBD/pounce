...
# TODO: 
# - adjust dpa a posteriori?
# - different spacing suction/pressure side?
# - loop over n_circ?
# - separate gmsh from movement?
...
def make_mesh(hopr_path,reference_mode,hoprbasefile,name_str,random_vars,n_avg):
   ...
   # do_show_fig = False
   do_show_fig = True
   do_plot_inner_lines = False
   inner_line_str = ":"
   n_steps_show = 5000

   # ax_limits=(-0.15,0.15,-0.15,0.15)
...
# --------------------------------------------------------------------------------
   ...
   if "wedge" in mesh.cells: 
       ...
       sys.exit()
   ind = mesh.cells["hexahedron"][:,:4]
   ind = np.concatenate((ind,ind[:,:1]),axis=1)
   x_all = mesh.points[:,0]
   x = x_all[ind]
   y_all = mesh.points[:,1]
   y = y_all[ind]
   ...
   linecol = [0.5,0.5,0.5]
   plt.plot(x_afl_n[:,0],x_afl_n[:,1],color=linecol)

   if do_save_fig: 
       # near field
       # fn = "figs_mesh/"+str(i_sample+1)+".png"
       # ax.axis(ax_limits)
       # fn = "figs_mesh/mf.png"
...
if __name__ == "__main__": 
   ...
   for i_sample in range(1,2):
       n_modes = 35
       random_vars = []
       for i_var in range(n_modes):
           seed = 100*i_sample + i_var + 50
           np.random.seed(seed)
           random_vars.append(np.random.uniform(-0.75,0.75,1))
       n_avg = 5
       make_mesh(hopr_path,reference_mode,hoprbasefile,name_str,random_vars,n_avg)
